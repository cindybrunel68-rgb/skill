...
Read current goals and priorities from Claude Code's native auto-memory: open `~/.claude/projects/-root-aios/memory/goals.md` (or whatever topic file holds them, check `MEMORY.md` index) with the Read tool. If goals haven't been captured yet, ask the user for them and write them to `goals.md`.
...
After the review:
1. Update `~/.claude/projects/-root-aios/memory/goals.md` with new priorities if they've changed (Write tool). If the topic file doesn't exist yet, create it and add a pointer line to `MEMORY.md`.
